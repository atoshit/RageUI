...
---@type table
local SettingsList = {
    Text = { X = 410, Y = 5, Scale = 0.28 },
    LeftArrow = { Dictionary = "commonmenu", Texture = "arrowleft", X = 340, Y = 8, Width = 18, Height = 18 },
    RightArrow = { Dictionary = "commonmenu", Texture = "arrowright", X = 390, Y = 8, Width = 18, Height = 18 },
}
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu then
            ...
            if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
                ...
                local ListText = (type(Items[Index]) == "table") and string.format("← %s%s~s~ →", RageUI.ColorFormat(), Items[Index].value) or string.format("← %s%s~s~ →", RageUI.ColorFormat(), Items[Index]) or "undefined"
                ...
                if Enabled == true or Enabled == nil then
                    if Selected then
                        ListText = (type(Items[Index]) == "table") and string.format("← ~s~%s~s~ →", Items[Index].value) or string.format("← ~s~%s~s~ →", Items[Index]) or "undefined"
                        ...
                        ListText = (type(Items[Index]) == "table") and string.format("← %s%s~s~ →", RageUI.ColorFormat(), Items[Index].value) or string.format("← %s%s~s~ →", RageUI.ColorFormat(), Items[Index]) or "undefined"
                    ...
                    if Style.Enabled == true or Style.Enabled == nil then
                        if type(Style) == 'table' then
                            if Style.LeftBadge ~= RageUI.BadgeStyle.None then
                                local BadgeData = Style.LeftBadge and Style.LeftBadge(Selected) or RageUI.BadgeStyle.Star(Selected)
